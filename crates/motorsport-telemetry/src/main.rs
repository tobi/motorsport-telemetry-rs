...
use telemetry_format::{
    ...
    write_jsonl_extension_from_source_with, write_jsonl_from_source_with, JsonlRecording,
    NativeRecording, FORMAT_VERSION,
};
...
Usage: motorsport-telemetry convert <input> [output]
...
fn main() {
    match arguments(std::env::args_os().skip(1)) {
        ...
        Ok(Command::Convert { input, output }) => match convert(&input, output.as_deref()) {
...
fn convert(
    input: &Path,
    output: Option<&Path>,
) -> Result<PathBuf, motorsport_telemetry::TelemetryError> {
    let dest = output
    ...
    if is_jsonl_path(&dest) {
        ...
        if is_jsonl_ext_path(&dest) {
            write_jsonl_extension_from_source_with(&file, &dest, compress)?;
        } else {
            write_jsonl_from_source_with(&file, &dest, compress)?;
        }
    } else {
